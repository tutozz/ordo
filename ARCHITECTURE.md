...
| `controle.py` | the reconciliation loop: `scope_drift`, `fausse_completion`, `wake_reasons`/`wake_new`, `tick()`, and `_compacter()` which flattens an over-long executor's context | let an exception raised while reconciling one campaign stop `tick()` for the others - each campaign is wrapped in its own try/except |
...
| `carte.py` | the read-only picture of a campaign: dependency levels, phases read from title prefixes, blocking reasons, warnings, the questions still waiting on the human, plus `vue()` (the flat shape the page consumes), `html()` (a self-contained page carrying that shape as JSON), `panneau()` (one campaign, fed by the server) and `page()` (the wall that lays those columns side by side and carries no campaign data at all) | write anything, to `state.json` or elsewhere (`cmd_map` owns the file write, this module only returns strings); talk to tmux - pane liveness is injected through `alive`, so a map can be drawn with no tmux server at all; invent a missing `why` - what nobody explained is reported as unexplained, never filled in; surface a question not marked `pourHumain`, or one already answered - the first belongs to the orchestrator, the second is an alarm that never goes off; compute layout - positions do not exist before the browser reflows the rows, so Python serves data and the page places it; concatenate campaign content into markup - it travels as JSON and reaches the DOM only through textContent |
| `situation.py` | the re-entry block a human reads cold: live phase and its progress, live tasks with their title and their `why`, what is waiting on the human, what is launchable next; `render()` turns it into the text an orchestrator copies into a message | emit a task id without its title on the same line - that is the entire contract of the module, and the reason it exists rather than a richer `show`; surface a question not marked `pourHumain` - that one belongs to the orchestrator, and ringing the human for it is a false alarm; hide a missing `why` behind a plausible sentence - an unexplained live task prints the command that repairs it |
| `usage.py` | tokens an executor actually spent, summed from its own Claude Code transcript (`~/.claude/projects/*/<claudeSessionId>.jsonl`), read incrementally and deduplicated per turn; `SEUIL_TOURS`, the measured session length past which a context is worth flattening | report zero for a transcript it cannot find - absent is absent, and a `running` task showing 0 tokens is a false measurement; re-read a multi-megabyte transcript from the start on every poll; derive the transcript's directory name from the project path - that encoding is not ours, the session id is unique, so it searches |
| `serveur.py` | the single local map server on port 9123: the registry of known `ORDO_HOME`s, liveness by signature, detached start, and the read-only HTTP routes (`/` the wall, `/panel` one column, `/api/state`, `/api/map`, `/health`) | write anything - there is no POST and no route mutates state; listen anywhere but `127.0.0.1`; trust the `home` query parameter (it is checked against the registry) or the `Host` header (checked against loopback names, which is what closes DNS rebinding); let its own failure break the watch that started it |
...
| `carte.py` | `chantier`, `journal`, `routage`, `store`, `usage` |
| `situation.py` | `carte`, `store` |
| `serveur.py` | `carte`, `chantier`, `panes`, `store`, `usage` |
| `controle.py` | `capteur`, `chantier`, `journal`, `panes`, `plan`, `report`, `store`, `usage` |
| `cli.py` | `capteur`, `chantier`, `journal`, `panes`, `plan`, `prompt`, `report`, `situation`, `store`, plus `controle`, `carte` and `serveur` imported lazily, inside `cmd_tick`, `_map_write` and `cmd_serve`/`cmd_watch` only, so the rest of the CLI keeps working if any of them is broken or absent |
