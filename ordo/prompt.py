...
def _section_interdits() -> str:
    return (
        ...
        "5. A SHARED RESOURCE IS NEVER TAKEN, RESTARTED OR KILLED - you bring up your own "
        "next to it. Several executors run at the same time on this machine and they share "
        "more than the working tree: one tmux server, one map server on :9123, one "
        "Playwright browser profile. Whenever one of them is busy or holds a lock, the "
        "reflex that looks efficient - kill it, restart it, take it over - destroys the "
        "work of a session you cannot see. This has cost three executors a full stop in a "
        "single day on the browser alone (`Browser is already in use`). What works, "
        "measured, every time: your own Chromium with a dedicated profile driven by a short "
        "script; your own map server on another port; your own pane killed by its own id. "
        "You start it, you use it, you shut it down. If you truly cannot bring up your own, "
        "report `blocked` and say exactly which resource is held - never take it.\n"
